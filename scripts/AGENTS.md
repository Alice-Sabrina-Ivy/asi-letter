...
| `check_ots_digest.py` | Verifies each `letter/*.asc.ots` commits to the SHA-256 of its `.asc`'s current bytes. | A missing proof passes (with a note) because `release.yml` verifies before it stamps; a proof of different bytes fails and is never restamped silently. v1.3.0's proof covered a superseded same-day upload. | Run by `verify-clearsign.sh`; closes the gap where re-uploading an `.asc` kept its old proof. |
