...
| `auto-release-latest-letter.yml` | On pushes to `letter/RELEASES.json` / `keys/**` / README / licenses, on dispatch from `ots-upgrade`, or manually: reads the newest entry in `letter/RELEASES.json`, ensures the `.ots` proof exists, and publishes a GitHub Release marked as latest. | Requires `jq` and GitHub CLI. Skips silently (green) unless the manifest's newest entry matches the newest letter on disk and has md/asc/ots paths that exist — so a stale manifest reads as "nothing to do". Own concurrency group `auto-release-*`, since it mutates the Releases API rather than the working tree. | Publishes the “latest recommended” release snapshot with attachments and verification notes. |
